...
const STATE = {
  apiSettings: {
    apiUrl: "", // Set Web App URL here
    isMockMode: true // Defaults to Mock Mode until configured
  },
  project: {
...
function loadSettings() {
  ...
  if (savedMode) STATE.apiSettings.isMockMode = (savedMode === "mock");
...
function handleSettingsFormSubmit(e) {
  ...
  const modeVal = document.querySelector('input[name="setting-mode"]:checked').value;
  ...
  if (modeVal === "online" && !urlVal) {
    alert("กรุณากรอก Web App URL หากเลือกโหมดออนไลน์");
    return;
  }
  
  STATE.apiSettings.isMockMode = (modeVal === "mock");
  STATE.apiSettings.apiUrl = urlVal;
  STATE.project.adminPassword = pwVal;
  
  localStorage.setItem("lent_api_url", urlVal);
  localStorage.setItem("lent_operation_mode", modeVal);
...
/* ==========================================================================
   4. DATE CONSTRAINT AND TEST MODE BYPASS
   ========================================================================== */

function initDateCheck() {
  const chk = document.getElementById("bypass-date-chk");
  chk.addEventListener("change", (e) => {
    updateSurveyAvailability(e.target.checked);
  });
  
  // Set default system date display
  document.getElementById("system-date-display").innerText = formatDateThai(STATE.project.systemDate);
  
  // Perform date check initially
  const isPast = new Date(STATE.project.systemDate) >= new Date(STATE.project.targetCompletionDate);
  updateSurveyAvailability(isPast);
}
...
function updateSurveyAvailability(enable) {
  ...
  if (enable) {
    ...
    warn.className = "alert alert-success";
    warn.querySelector("p").innerHTML = `🎉 **โหมดทำแบบสอบถามเปิดแล้ว** (ระบบอนุญาตให้เข้าทำแบบสอบถามได้ตามเงื่อนไข)`;
  } else {
    container.classList.add("disabled-container");
    inputEl.disabled = true;
    searchBtn.disabled = true;
    warn.className = "alert alert-warning";
    warn.querySelector("p").innerHTML = `⚠️ **แจ้งเตือนขอบเขตโครงการ:** ขณะนี้วันที่ปัจจุบันยังไม่ถึงวันที่ **26 ส.ค. 2569** (วันครบกำหนดโครงการเข้าพรรษา 90 วัน) ระบบกรอกข้อมูลจะเปิดใช้งานในวันดังกล่าว`;
    resetSurveyForm();
  }
}
