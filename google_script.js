...
function getSS() {
  try {
    ...
    throw new Error("Failed to open spreadsheet by ID '" + SPREADSHEET_ID + "': " + err.toString());
  }
}
...
function initSheets() {
  ...
  if (!configSheet) {
    ...
    configSheet.appendRow(["SurveyOpen", "false"]);
  }
...
function getSystemConfig(sheet) {
  if (!sheet) return { SurveyOpen: "false" };
  var data = sheet.getDataRange().getValues();
  var config = {};
  for (var i = 1; i < data.length; i++) {
    config[data[i][0]] = data[i][1].toString().trim();
  }
  if (!config.hasOwnProperty("SurveyOpen")) {
    config.SurveyOpen = "false";
  }
  return config;
}

// Update Config in Sheet
function handleSetSurveyStatus(ss, status) {
  var sheet = ss.getSheetByName("Config");
  if (!sheet) {
    sheet = ss.insertSheet("Config");
    sheet.appendRow(["Key", "Value"]);
    sheet.appendRow(["SurveyOpen", "false"]);
  }
  var data = sheet.getDataRange().getValues();
  var rowIdx = -1;
  for (var i = 1; i < data.length; i++) {
  ...
  if (rowIdx !== -1) {
    sheet.getRange(rowIdx, 2).setValue(status);
  } else {
    sheet.appendRow(["SurveyOpen", status]);
  }
...
function doGet(e) {
  ...
  if (ss && (!ss.getSheetByName("Name") || !ss.getSheetByName("Registrations") || !ss.getSheetByName("Surveys") || !ss.getSheetByName("Winners") || !ss.getSheetByName("Config"))) {
    initSheets();
  }
  ...
  try {
    if (action === "getAllData") {
      responseData = {
        ...
        data: {
          ...
          config: getSystemConfig(ss.getSheetByName("Config"))
        }
      };
    ...
    } else if (action === "setSurveyStatusJSONP") {
      var status = e.parameter.status;
      responseData = handleSetSurveyStatus(ss, status);
    } else {
